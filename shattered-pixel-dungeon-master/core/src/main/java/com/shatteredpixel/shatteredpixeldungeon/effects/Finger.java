...
public class Finger extends Group {
	...
	public Finger(int from, int to, Callback callback){
		this(DungeonTilemap.tileCenterToWorld(from),
				DungeonTilemap.tileCenterToWorld(to),
				callback);
	}

	public Finger(PointF from, PointF to, Callback callback){
		super();
		...
		for (int i = 0; i < chains.length; i++){
			chains[i] = new Image(Effects.get(Effects.Type.FINGER));
