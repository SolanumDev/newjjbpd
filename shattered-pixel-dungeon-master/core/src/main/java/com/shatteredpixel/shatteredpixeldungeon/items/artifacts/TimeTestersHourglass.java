...
public class TimeTestersHourglass extends Artifact {
	...
	public static Mob worldEnterer = null;

	//keeps track of generated sandbags.
	public int sandBags = 0;

	public static CellSelector.Listener TIMESTOPPER = new CellSelector.Listener() {

		@Override
		public void onSelect(Integer cell) {

			if(cell!= null && Actor.findChar(cell) != null)
			{
			    worldEnterer = (Mob) Actor.findChar(cell);
			}
			else
            {
                GLog.w("You must select a tile with a mob!");
            }
		}
        ...
        public String prompt() {
            ...
            return "What will invade your _World_ of stopped time?";
        }
    ...
    //TIMESTOPTODO:
	//all actors check if time is stopped
		//if time is stopped and we have the time stop debuff don't move
			//in the case of blobs simply don't evolve; skip your turn
            //in the case of buffs skip your turn (without decrementing)
		//inside the level handler check the time stop variable
		//if time is stopped do not activate traps until time is no longer stopped
					...
					GLog.i( Messages.get(this, "deactivate") );
						...
						new WndOptions( Messages.get(this, "name"),
							...
							protected void onSelect(int index) {
								if (index == 0) {
									GLog.i( Messages.get(TimeTestersHourglass.class, "onstasis") );
									...
									//GameScene.selectCell(TIMESTOPPER);
									GLog.i( Messages.get(TimeTestersHourglass.class, "onfreeze") );
									GameScene.flash(0xFFFFFF);
									Sample.INSTANCE.play(Assets.SND_TELEPORT);
	...
	public static class sandBag extends Item {
		...
		public boolean doPickUp( Hero hero ) {
			TimeTestersHourglass hourglass = hero.belongings.getItem( TimeTestersHourglass.class );
